...
app.use('/*', cors());
const getStockQuote = tool(async ({ symbol, companyName }) => {
    const quote = await lookupStockQuote({ symbol, companyName });
    return JSON.stringify(quote);
}, {
    name: 'get_stock_quote',
    description: 'Fetch the latest stock quote from Alpha Vantage for a ticker symbol or company name when the user asks for live stock data.',
    schema: z.object({
        symbol: z.string().trim().min(1).optional(),
        companyName: z.string().trim().min(1).optional(),
    }),
});
const getStockTimeSeries = tool(async ({ symbol, companyName, window }) => {
    const series = await lookupStockTimeSeries({ symbol, companyName, window });
    return JSON.stringify(series);
}, {
    name: 'get_stock_time_series',
    description: 'Fetch a recent stock price time series from Alpha Vantage for a ticker symbol or company name. Use this for trend, chart, day-vs-week-vs-month, or performance-over-time questions.',
    schema: z.object({
        symbol: z.string().trim().min(1).optional(),
        companyName: z.string().trim().min(1).optional(),
...
const getNewsArticle = tool(async ({ symbol, companyName, query }) => {
    const article = await lookupNewsArticle({ symbol, companyName, query });
    return JSON.stringify(article);
}, {
    name: 'get_news_article',
    description: 'Fetch a recent article from NewsData.io when the user asks for the latest news, headlines, or an article about a company, ticker, or topic.',
    schema: z.object({
        symbol: z.string().trim().min(1).optional(),
        companyName: z.string().trim().min(1).optional(),
...
`;
function createRoomScopedAgent(roomName) {
    const getRoomContext = tool(async () => {
    ...
    const searchUploadedDocument = tool(async ({ query }) => {
        ...
        description: 'Search the latest ingested uploaded financial document for the active room and return the most relevant chunks for a user question.',
        schema: z.object({
            query: z.string().min(3),
        }),
    });
    const projectStockFromEarningsCall = tool(async ({ symbol, companyName, queryContext }) => {
        const latestDocument = await readLatestReadyDocument(roomName);
        if (!latestDocument) {
            return JSON.stringify({
        ...
        description: 'Use the latest uploaded earnings-call PDF plus a 1-month stock time series to produce a grounded stock projection with supporting evidence excerpts.',
        schema: z.object({
            symbol: z.string().trim().min(1).optional(),
            companyName: z.string().trim().min(1).optional(),
...
function inferStockToolHint(transcript) {
    const normalized = transcript.toLowerCase();
    const wantsProjection = /\b(projection|project|forecast|outlook|guidance|expected move)\b/.test(normalized) &&
    ...
    const wantsTimeSeries = /\b(chart|time series|trend|trending|intraday|historical|history|performance|series)\b/.test(normalized) ||
        /\b(1 day|one day|today|intraday|5 day|five day|5 days|five days|1 month|one month|month)\b/.test(normalized);
    if (!wantsTimeSeries) {
        return null;
    }
    let window = '5d';
...
function extractEmbeddedTimeSeriesResult(result) {
    try {
        ...
        if (timeSeries.kind !== 'stock_time_series' ||
...
async function* sttStream(context, audioStream) {
    let stt;
    try {
        ...
        yield createVoiceEvent(context, {
            type: 'error',
            stage: 'stream',
    ...
    const producer = iife(async () => {
        try {
            ...
            passthrough.push(createVoiceEvent(context, {
                type: 'error',
                stage: 'stream',
        ...
        try {
            for await (const event of stt.receiveEvents()) {
                console.log('[meet/voice-agent-server] stt event', {
                    ...
                    sessionId: context.sessionId,
                    type: event.type,
                    transcript: event.type === 'stt_chunk' || event.type === 'stt_output'
...
async function* agentStream(context, eventStream) {
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                if (ToolMessage.isInstance(message)) {
                    const normalizedResult = typeof message.content === 'string' ? message.content : JSON.stringify(message.content);
...
async function* ttsStream(context, eventStream) {
    const passthrough = writableIterator();
    let tts = null;
    let ttsUnavailableMessage = null;
    let ttsConsumerPromise = null;
    ...
    return {
        onOpen(_, ws) {
            ...
            flushPromise = iife(async () => {
                try {
                    ...
                    currentSocket?.send(JSON.stringify(createVoiceEvent(context, {
                        type: 'error',
                        stage: 'stream',
        ...
        onMessage(event) {
            ...
            console.log('[meet/voice-agent-server] websocket message unhandled', {
                ...
                constructorName: typeof data === 'object' && data !== null && 'constructor' in data
        ...
        async onError(error, ws) {
            ...
            ws.send(JSON.stringify(createVoiceEvent(context, {
                type: 'error',
                stage: 'stream',
