...
type AlphaVantageIntradayResponse = {
  'Time Series (30min)'?: Record<string, { '4. close'?: string }>;
};
...
type TimeSeriesWindow = '1d' | '5d' | '1m';
...
export type StockProjection = {
  ...
  projectedAbsoluteChange: number;
  projectedPercentChange: string;
  slopePerTradingStep: number;
  method: string;
};
...
async function resolveSymbol(companyName: string) {
  const payload = (await fetchAlphaVantage({
    function: 'SYMBOL_SEARCH',
    keywords: companyName,
    apikey: getApiKey(),
  })) as AlphaVantageSymbolSearchResponse;
...
export async function lookupStockQuote({ symbol, companyName }: LookupArgs) {
  const resolved =
    symbol && symbol.trim()
      ? {
  ...
  return {
    ...
    summary: `${quote['01. symbol']} is trading at $${price.toFixed(2)}${
      quote['10. change percent'] ? ` with a ${quote['10. change percent']} move on the day` : ''
    }.`,
  };
}

function formatWindowLabel(window: TimeSeriesWindow) {
  switch (window) {
    case '1d':
      return '1 day';
    case '5d':
...
function formatSeriesPointLabel(timestamp: string, window: TimeSeriesWindow) {
...
function resolvePercentChange(current: number, previous: number) {
  if (previous === 0) {
    return '0.00%';
  }
...
function resolveTimeSeriesPoints(
  entries: Array<[string, { '4. close'?: string }]>,
  ...
  return entries
    ...
    .filter((point): point is StockTimeSeriesPoint => point !== null);
}

export function projectStockFromPoints(points: Array<{ value: number }>): StockProjection {
  if (points.length < 2) {
...
export async function lookupStockTimeSeries({
  symbol,
  companyName,
  window,
}: LookupArgs & { window?: TimeSeriesWindow }) {
  ...
  if (resolvedWindow === '1d') {
    ...
    const entries = Object.entries(payload['Time Series (30min)'] ?? {}).sort((left, right) =>
    ...
    return {
      kind: 'stock_time_series' as const,
      symbol: resolved.symbol,
      companyName: resolved.companyName,
  ...
  const entries = Object.entries(payload['Time Series (Daily)'] ?? {}).sort((left, right) =>
    right[0].localeCompare(left[0]),
  );
  const points = resolveTimeSeriesPoints(entries, resolvedWindow);
...
export async function lookupStockProjection(args: LookupArgs) {
  ...
  return {
    kind: 'stock_projection' as const,
